...
describe('json-format settings regression guards', () => {
    it('uses explicit defaults for content-script json-format options', () => {
        ...
        expect(source).toContain("storageQuery = ['FH_JSONFORMAT_DEFAULTS_MIGRATED', 'FH_JSONFORMAT_FIX_ENCODING_RESTORED'].concat(params);");
        expect(source).toContain("storageQuery = ['FH_JSONFORMAT_DEFAULTS_MIGRATED', 'FH_JSONFORMAT_FIX_ENCODING_RESTORED', params];");
        expect(source).toContain('FH_JSONFORMAT_FIX_ENCODING_RESTORED: false');
        ...
        expect(source).toContain("const fixEncodingRestored = String(result.FH_JSONFORMAT_FIX_ENCODING_RESTORED) === 'true';");
        ...
        expect(source).toContain("result.FIX_ERROR_ENCODING = true;");
        expect(source).toContain("FIX_ERROR_ENCODING: 'true'");
        expect(source).toContain("FH_JSONFORMAT_FIX_ENCODING_RESTORED: 'true'");
        expect(source).toContain("ENABLE_JSON_KEY_SORT: 'true'");
        expect(source).toContain("NESTED_ESCAPE_PARSE: 'false'");
    ...
    it('manual json-format page no longer silently inherits legacy nested-parse keys', () => {
        ...
        expect(html).toContain('class="fh-window-note-popover"');
        expect(html).toContain('v-if="windowNoteEditorOpen"');
        expect(html).toContain('v-model="windowNote"');
        expect(html).toContain('@input="syncWindowNote"');
        expect(html).toContain('@click.stop="toggleWindowNoteEditor"');
        expect(css).toContain('.fh-window-note-control');
        expect(css).toContain('.fh-window-note-trigger');
        expect(css).toContain('.fh-window-note-popover');
        ...
        expect(source).toContain("windowNoteEditorOpen: false");
        ...
        expect(source).toContain('toggleWindowNoteEditor()');
        expect(source).toContain('handleWindowNoteOutsideClick(event)');
        ...
        expect(css).toContain('.fh-json-tool .fh-lite-sort-option input:checked + span');
        ...
        expect((html.match(/恢复原 JSON/g) || []).length).toBe(1);
        expect(html).not.toContain('class="fh-pane-actions" v-if="nodeEditActive"');
        expect(css).toContain('.fh-node-edit-banner .fh-node-restore-action');
        ...
        expect(html).toContain('class="fh-lite-sort-trigger"');
        expect(html).toContain('<span>排序</span><strong>{{sortLabel}}</strong><i aria-hidden="true"></i>');
        expect(html).toContain('class="fh-lite-sort-options"');
        expect(html).toContain('for="sort_asc_lite"');
        expect(html).toContain('for="sort_desc_lite"');
        expect(html).toContain('for="sort_asc"');
        expect(html).toContain('for="sort_desc"');
        expect(html).toContain('name="jsonsort" id="sort_null" value="0" v-model="sortType" @change="handleSortChange"');
        expect(html).toContain('name="jsonsort" id="sort_asc" value="1" v-model="sortType" @change="handleSortChange"');
        expect(html).toContain('name="jsonsort" id="sort_desc" value="-1" v-model="sortType" @change="handleSortChange"');
        expect(html).toContain('name="jsonsort" id="sort_asc_lite" value="1" v-model="sortType" @change="handleSortChange"');
        expect(html).toContain('name="jsonsort" id="sort_desc_lite" value="-1" v-model="sortType" @change="handleSortChange"');
        ...
        expect(html).not.toContain('退出原文预览');
        expect(html).not.toContain('@click="toggleRawFallbackResult"');
        expect(html).toContain('关闭嵌套解析后重试');
        expect(source).not.toContain('RAW_FALLBACK_PREVIEW_LIMIT');
        expect(source).not.toContain('buildRawFallbackHtml(source, options = {})');
        expect(source).not.toContain('showRawFallbackResult(full = false)');
        expect(source).not.toContain('toggleRawFallbackResult()');
        expect(source).not.toContain('exitRawFallbackResult()');
        expect(source).not.toContain('showFullRawFallbackResult()');
        expect(source).not.toContain('rawFallbackReturnHtml');
        expect(source).not.toContain('rawFallbackTruncated');
        expect(source).not.toContain('class="fh-raw-fallback-meta"');
        expect(source).toContain('原文已保留在左侧输入框');
        expect(source).not.toContain('这里不做解析');
        ...
        expect(css).toContain('html.fh-jf .fh-json-error-actions .fh-error-tool-action');
        expect(css).toContain('border: 1px solid rgba(37, 99, 235, 0.16) !important;');
    ...
    it('content-script scopes advanced auto-format controls to Omni mode', () => {
        ...
        expect(source).toContain('class="fh-sort-trigger"');
        expect(source).toContain('id="fhJsonSortCurrent"');
        expect(source).toContain('class="fh-sort-options"');
        expect(source).toContain('class="fh-radio-pill fh-sort-option" for="sort_null"');
        expect(source).toContain('class="fh-radio-pill fh-sort-option" for="sort_asc"');
        expect(source).toContain('class="fh-radio-pill fh-sort-option" for="sort_desc"');
        expect(source).toContain('let _syncSortMenuState = () => {');
        expect(source).toContain("_syncSortMenuState();");
        expect(source).toContain('id="jsonGetCorrectCnt">乱码修正</button>');
        expect(source).toContain('class="fh-search-menu"');
        expect(source).toContain('class="fh-search-trigger"');
        expect(source).toContain('id="fhJsonSearchTrigger"');
        expect(source).toContain('class="fh-search-popover"');
        expect(source).toContain('class="fh-node-actions"');
        expect(source).toContain('id="fhJsonSearchInput"');
        expect(source).toContain("$('#fhJsonSearchTrigger').off('click.fhOmni')");
        ...
        expect(cssSource).toContain('width: 168px;');
        expect(cssSource).toContain('body.fh-json-compact .x-toolbar.fh-json-viewbar {\n    min-height: 38px;');
        expect(cssSource).toContain('html.fh-jf .fh-sort-options {\n    position: absolute;');
        expect(cssSource).toContain('align-items: stretch;');
        expect(cssSource).toContain('html.fh-jf .fh-viewbar-main {\n    flex: 1 1 auto;\n    min-width: 0;\n    gap: 8px;\n    overflow-x: visible;\n    overflow-y: visible;');
        expect(cssSource).toContain('pointer-events: none;');
        expect(cssSource).toContain('visibility: hidden;');
        expect(cssSource).toContain('html.fh-jf .fh-sort-menu:hover .fh-sort-options,\nhtml.fh-jf .fh-sort-menu:focus-within .fh-sort-options');
        expect(cssSource).toContain('html.fh-jf body.theme-dark .fh-sort-options');
        expect(cssSource).toContain('html.fh-jf .fh-search-popover {\n    position: absolute;');
        expect(cssSource).toContain('html.fh-jf .fh-search-menu:hover .fh-search-popover,\nhtml.fh-jf .fh-search-menu:focus-within .fh-search-popover');
        expect(cssSource).toContain('html.fh-jf .fh-node-actions');
        expect(cssSource).toContain('html.fh-jf body.theme-dark .fh-search-popover');
    ...
    it('Issue #603: JSONPath extraction returns copyable JSON payloads and row actions', () => {
        ...
        expect(css).not.toContain('.fh-raw-fallback-pre');
        expect(css).not.toContain('.fh-raw-fallback-meta');
    });

    it('Issue #628/#629/#632: JSON format node actions and parse fallback are explicit', () => {
        const html = readSource('apps/json-format/index.html');
        const css = readSource('apps/json-format/index.css');
        const contentCss = readSource('apps/json-format/content-script.css');
        const formatLib = readSource('apps/json-format/format-lib.js');

        expect(html).toContain('class="fh-node-edit-banner"');
        expect(html).toContain('正在编辑选中节点');
        expect(html).toContain('@click="restoreNodeEditSource"');
        expect(formatLib).toContain('class="opt-path"');
        expect(formatLib).toContain('当前节点 JSONPath 已复制');
        expect(formatLib).toContain('let _renderRawParseFallback = function(jsonStr, error)');
        expect(formatLib).toContain('RAW_PARSE_FALLBACK_PREVIEW_LIMIT');
        expect(formatLib).toContain('let previewRaw = raw.length > RAW_PARSE_FALLBACK_PREVIEW_LIMIT');
        expect(formatLib).toContain('JSON 解析失败，已保留原文');
        expect(formatLib).toContain('fh-raw-source-fallback-meta');
        expect(formatLib).toContain('_renderRawParseFallback(jsonStr, e);');
        expect(formatLib).toContain('let text = _safeStringify(value, 4);');
        expect(css).toContain('.fh-node-edit-banner');
        expect(css).toContain('.fh-raw-source-fallback');
        expect(css).toContain('.fh-raw-source-fallback-meta');
        expect(contentCss).toContain('.fh-raw-source-fallback');
        expect(contentCss).toContain('.fh-raw-source-fallback-meta');
    });
});
